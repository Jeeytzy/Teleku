...
function formatterMessage(conn, m, store) {
  ...
  if (m.message) {
    m.mtype = getContentType(m.message);
     m.msg = (m.mtype === 'conversation' && m.message.conversation) ? m.message.conversation : (m.mtype === 'imageMessage') && m.message.imageMessage.caption ? m.message.imageMessage.caption : (m.mtype === 'videoMessage') && m.message.videoMessage.caption ? m.message.videoMessage.caption : (m.mtype === 'extendedTextMessage') && m.message.extendedTextMessage.text ? m.message.extendedTextMessage.text : (m.mtype === 'buttonsResponseMessage') && m.quoted && m.message.buttonsResponseMessage.selectedButtonId ? m.message.buttonsResponseMessage.selectedButtonId : (m.mtype === 'templateButtonReplyMessage') && m.quoted.fromMe && m.message.templateButtonReplyMessage.selectedId ? m.message.templateButtonReplyMessage.selectedId : (m.mtype === 'messageContextInfo') ? (m.message.buttonsResponseMessage?.selectedButtonId || m.message.listResponseMessage?.singleSelectReply.selectedRowId) : (m.mtype == 'listResponseMessage') && m.quoted && m.message.listResponseMessage.singleSelectReply.selectedRowId ? m.message.listResponseMessage.singleSelectReply.selectedRowId : "" || (m.mtype == 'viewOnceMessage' ? m.message[m.mtype].message[getContentType(m.message[m.mtype].message)] : m.message[m.mtype]);
...
async function cleanCacheSession(dir) {
    try {
      ...
      for (const ent of entries) {
        ...
        if (ent.isFile()) {
          if (ent.name !== 'creds.json') {
            fs.unlink(full, () => {});
          }
        } else if (ent.isDirectory()) {
          await clean(full);
          const remain = await new Promise((resolve) => {
            fs.readdir(full, (err, files) => resolve(files || []));
          });
          if (remain.length === 0) {
            fs.rmdir(full, () => {});
          }
