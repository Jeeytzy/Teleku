...
async function startServer() {
  try {
    ...
    const checkSessionExists = checkFileExists('./session', 'creds.json');

    if (checkSessionExists) {
      console.log('Session folder detected exists, if the server is still not connected to your WhatsApp, please delete the session folder first, then try running the server again!');
    }
    ...
    if (config.pairing_mode && !checkSessionExists && !socket.authState.creds.registered) {
      ...
      setTimeout(async () => {
        let code = await socket.requestPairingCode(phoneNumber, 'JEEYVIPP');
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (reason === DisconnectReason.badSession) {
          ...
          socket.logout();
          deleteFolder(path.join(__dirname, 'session'))
      ...
      //await cleanCacheSession('./session');
    ...
    socket.ev.on('messages.upsert', async (chatUpdate) => {
      try {
        ...
        require('./lib/handler.js')(socket, messages, memoryStore);
      } catch (error) {
        console.error(error);
      }
    });
